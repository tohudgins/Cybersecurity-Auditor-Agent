...
### `retrieval/`
- `retriever.py` — hybrid retrieval. `retrieve(query, frameworks, k)` runs three searches **concurrently** (a `ThreadPoolExecutor` — vector waits on a network embed while BM25 is CPU-bound, so wall-clock ≈ the slowest) and fuses them via Reciprocal Rank Fusion: (1) exact `control_id` metadata lookup, (2) Chroma vector similarity, (3) BM25 keyword search (top-k via `np.argpartition`, not a full sort over ~40k scores). **Framework-scoped:** when the caller doesn't pin `frameworks`, `framework_router.detect_frameworks(query)` auto-scopes to the framework(s) the query names (so "SOC 2 CC6.1" retrieves SOC 2 TSC, not whichever standard happens to be semantically close) — at 56 frameworks this is what keeps precision. A cheap deterministic **re-rank** (`_rerank`: exact-ID match, named-framework boost, lexical overlap) orders the fused pool before trimming to k — no model call, so precision without latency. Framework filter applies post-hoc to BM25 (BM25 has no metadata filter). The BM25 index is also persisted to `.chromadb/bm25_cache.pkl`, keyed on the collection's chunk count, so a fresh process loads it instead of re-pulling/re-tokenizing the whole corpus on the first query (a rebuild changes the count and transparently invalidates the cache; delete the file to force a rebuild otherwise). `get_vectorstore()` / `get_embeddings()` are `lru_cache`d so a single `retrieve()` reuses one Chroma client + embeddings object instead of constructing three.
- `_tokenize` strips trailing `.:`-style punctuation so `AC-2.` tokenizes as `ac-2` and BM25 hits exact control IDs.

### `tools/`
- `compliance_qa.py` — hybrid retrieval + LLM synthesis with citations. `answer_compliance_question()` / `stream_compliance_answer()` both take an optional `history` (recent `(role, content)` turns) so **multi-turn** follow-ups resolve against the thread; `_retrieval_query` expands a terse follow-up with the prior user turn so retrieval still lands. **Mapping/comparison questions** get the authoritative structured crosswalk injected via `mappings.crosswalk_context()` (so "map AC-2 to CIS/PCI" grounds on the crosswalk, not just prose). Synthesis runs `synthesis_model` (gpt-5.5) at `settings.synthesis_reasoning_effort` (default `"low"`).
- `control_advisor.py` — the **advisory-audit** path (a second *way to audit*). Organizational/process controls (governance, personnel, awareness training, incident-response *process*, vendor risk, physical security, contingency) are examined + interviewed, not scanned. Two stages, both grounded via RAG at `audit_model` (gpt-5.4-mini — structured tasks, ~2.5× faster than gpt-5.5 with equal quality): (1) `advise_controls(topic, frameworks)` → a `ControlAdvisory` **worksheet** (applicable controls, objective, interview questions, evidence to request, common gaps, citations); (2) `assess_organizational_controls(topic, responses, frameworks)` **closes the loop** — given the auditee's answers it returns an `OrganizationalAssessment` (per-control **interview-method** verdicts: satisfied/partial/not-satisfied/not-assessed + rationale + evidence gap + posture summary). Chat routing (`intake.is_advisory_request` + `looks_like_responses`): an advisory request with answers inline → assessment; without → worksheet (and `pending_advisory` is stashed so a follow-up reply of answers → assessment). Fixture-tested (`tests/test_control_advisor.py`, `tests/test_qa_enrichment.py`).
...
### `assessment.py`
- Control-coverage assessment layer — turns findings into a per-control verdict. `assess_controls(findings, artifact_kinds, scope)` returns `(list[ControlAssessment], CoverageSummary)`. Each artifact kind declares which controls its checks exercise (`_KIND_COVERAGE`); a control with findings → `not-satisfied` (`partial` if worst severity ≤ low), a covered control with no findings → `satisfied`, anything else in the baseline → `not-assessed`. Method is `test` for executing scanners (config/log/codebase) and `examine` for inspection (policy/text), per SP 800-53A. Findings resolve to their NIST anchor directly or via `mapped_controls`. **The baseline is `scope.baseline`**: `auditor-curated` (the crosswalk catalog), a named NIST 800-53B set (`low`/`moderate`/`high`), or an **alternative framework** (`cis`/`pci`/`csf`/`soc2`). NIST baselines are assessed directly; framework baselines are assessed by **projecting** each finding's NIST anchor onto the framework's controls via the crosswalk (`_framework_assessment_maps` → `project_to_framework` + `project_framework_id`) — coverage bounded by crosswalk completeness (surfaced in the report's limitations). Default is `settings.default_baseline` (`moderate`). Called in `audit_node` after `normalize_findings`; results flow to the reporting node and OSCAL export. `merge_interview_verdicts(assessments, coverage, verdicts)` **unifies the two ways to audit**: it folds an advisory (interview-method) assessment's per-control verdicts into a technical assessment — a process control the scanners left `not-assessed` takes the interview verdict (method `interview`); a control the scanners already exercised keeps its `test`/`examine` verdict; coverage is recomputed. The UI merges an advisory assessment into the last audit's coverage and shows a "Unified Control Coverage" delta.

### `scope.py`
- **Scope completeness** — the SSP layer. `AuditScope` is now a lightweight SSP: `system_name`/`description`/`owner`/`authorization_boundary` + an **asset inventory** (`assets: list[Asset]`, each `name`/`kind`/`description`). `assess_scope_coverage(scope, artifacts)` maps each in-scope asset to whether an artifact assessed it (by name reference first, then by kind, with `_KIND_ALIASES` so `web_app`→`target_url` etc.), returning a `ScopeCoverage`. The reporting node renders a **System Profile & Scope** section: system info + an asset table flagging in-scope assets with no evidence ("in scope but not assessed") — asset-level coverage that complements the control-level coverage. Fixture-tested (`tests/test_scope_and_merge.py`).
...
- **Compliance** (no targets, no prior audit): `stream_compliance_answer(question, frameworks, history)` — threads recent turns for multi-turn context.
