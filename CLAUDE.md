...
### `enrichment/`
- `catalog.py` — NIST SP 800-53 Rev. 5 **control catalog** (`data/baselines/control_catalog.json`, id→title) + **named impact baselines** (`data/baselines/baselines.json`). `catalog_title(id)` resolves any control (enhancements fall back to their base) so an assessment row never renders blank. `baseline_control_ids(name)` resolves a named baseline (`low`/`moderate`/`high`) to its full control set; baselines **compose additively** (`extends` chain: high→moderate→low), de-dup, and every member is validated against the catalog (dangling ids dropped, never invented). `control_title()` in `mappings.py` falls back here. Data-only to extend; fixture-tested (`tests/test_catalog.py`).
...
- **Single source of truth for config**: `src/auditor/config.py` (`Settings` via `pydantic-settings`). Don't hardcode model names, `.chromadb` path, or `data/` path elsewhere. `default_baseline` (`AUDITOR_DEFAULT_BASELINE`, default `moderate`) selects the assessment denominator when a run doesn't pass an explicit `scope`.
